...
// Use service role key if available, fallback to public publishable key
...
module.exports = async (req, res) => {
  ...
  // Webhook structure: { type: 'INSERT', table: 'notifications', record: { user_id, message, type } }
  if (payload && payload.record && payload.table === 'notifications') {
    const { user_id, message, type } = payload.record;
    
    // Query the target user's push subscription from the Supabase profiles table
  ...
  // 3. Broadcast to all active subscriptions (matching legacy server.js behavior)
  else {
    ...
    notificationTitle = (payload && payload.title) || notificationTitle;
    notificationBody = (payload && payload.body) || notificationBody;
    notificationUrl = (payload && payload.url) || notificationUrl;
  }
  ...
  const sendPromises = targetSubscriptions.map(async ({ userId, subscription }) => {
    try {
      ...
      console.error(`[Push serverless] Failed to send push to endpoint: ${subscription.endpoint}`, err.message);
      failed++;
      // Clean up stale subscription from database if expired/gone (e.g. Status 410 or 404)
